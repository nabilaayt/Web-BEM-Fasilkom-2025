...
const RelasiPage = () => {
  ...
  useEffect(() => {
    const fetchRelasiData = async () => {
      try {
        // Fetch all bidang to find Relasi
        const bidangList = await dinasService.getAllBidang();
        const relasiBidang = bidangList.find(bidang => 
          bidang.nama_bidang.toLowerCase().includes('relasi')
        );

        if (relasiBidang) {
          // Fetch all kategori dinas for Relasi bidang
          const kategoriList = await dinasService.getAllKategoriDinas();
          const relasiKategoris = kategoriList.filter(k => k.bidangId === relasiBidang.id);
          
          // Separate KoorBid and Dinas
          const koorbidKategoris = relasiKategoris.filter(k => 
            k.nama_dinas.toLowerCase().includes('koorbid')
          );
          const dinasKategoris = relasiKategoris.filter(k => 
            !k.nama_dinas.toLowerCase().includes('koorbid')
          );
      ...
      <div className="py-8">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg max-w-3xl mx-auto">
            Bidang yang bertanggung jawab dalam membangun dan memelihara hubungan baik dengan berbagai pihak eksternal, organisasi kemahasiswaan, dan stakeholder lainnya.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
          ...
          <div className="mb-16">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">
                Pimpinan yang mengkoordinasikan seluruh kegiatan di bidang Relasi
              </p>
            </div>
        ...
        {koorbidMembers.length === 0 && dinasList.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">Belum ada data untuk bidang Relasi</p>
          </div>
        )}
      </div>
    </div>
