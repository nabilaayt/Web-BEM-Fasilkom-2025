...
const SosialPolitikPage = () => {
  ...
  useEffect(() => {
    const fetchSosialPolitikData = async () => {
      try {
        // Fetch all bidang to find Sosial Politik
        const bidangList = await dinasService.getAllBidang();
        const sospolBidang = bidangList.find(bidang => 
          bidang.nama_bidang.toLowerCase().includes('sosial') ||
          bidang.nama_bidang.toLowerCase().includes('politik') ||
          bidang.nama_bidang.toLowerCase().includes('sospol')
        );

        if (sospolBidang) {
          // Fetch all kategori dinas for Sosial Politik bidang
          const kategoriList = await dinasService.getAllKategoriDinas();
          const sospolKategoris = kategoriList.filter(k => k.bidangId === sospolBidang.id);
          
          // Separate KoorBid and Dinas
          const koorbidKategoris = sospolKategoris.filter(k => 
            k.nama_dinas.toLowerCase().includes('koorbid')
          );
          const dinasKategoris = sospolKategoris.filter(k => 
            !k.nama_dinas.toLowerCase().includes('koorbid')
          );
        ...
        console.error('Error fetching Sosial Politik data:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchSosialPolitikData();
  }, []);
      ...
      <div className="py-8">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg max-w-3xl mx-auto">
            Bidang yang bertanggung jawab dalam pengembangan aspek sosial dan politik mahasiswa, advokasi, serta membangun relasi dengan berbagai pihak untuk kemajuan civitas akademika.
          </p>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-16">
          ...
          <div className="mb-16">
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">
                Pimpinan yang mengkoordinasikan seluruh kegiatan di bidang Sosial Politik
              </p>
            </div>
            ...
            <div className="text-center mb-12">
              ...
              <p className="text-gray-600 max-w-2xl mx-auto">
                {dinas.nama_dinas && dinas.nama_dinas.toLowerCase() === 'adper' 
                  ? 'Dinas yang mengelola administrasi dan perencanaan program kerja, serta koordinasi antar divisi dalam bidang Sosial Politik.'
        ...
        {koorbidMembers.length === 0 && dinasList.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">Belum ada data untuk bidang Sosial Politik</p>
          </div>
        )}
      </div>
    </div>
...
export default SosialPolitikPage;
