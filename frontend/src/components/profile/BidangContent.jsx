...
const BidangContent = ({ bidangSlug }) => {
  ...
  const bidangConfigs = {
    'inti': {
      title: 'Bidang Inti',
      description: 'Inti bertanggung jawab atas kebijakan, koordinasi antarbidang, serta memastikan program kerja berjalan sesuai visi dan misi BEM KM Fasilkom UNSRI.',
      keywords: ['inti'],
      priorityOrder: ['ketua', 'wakil']
    },
    'kerumahtanggaan': {
      title: 'Bidang Kerumahtanggaan',
      description: 'Bidang yang bertanggung jawab dalam pengelolaan administrasi, keuangan, dan pengembangan SDM di lingkungan BEM.',
      keywords: ['kerumahtanggaan', 'kerumah tanggaan', 'kerumah-tanggaan']
    },
    'sosial-politik': {
      title: 'Bidang Sosial Politik',
      description: 'Bidang yang fokus pada isu-isu sosial, politik, dan advokasi mahasiswa serta pengabdian masyarakat.',
      keywords: ['sosial politik', 'sosial-politik', 'sospol']
    },
    'relasi': {
      title: 'Bidang Relasi',
      description: 'Bidang yang menjalin hubungan eksternal dengan berbagai pihak untuk mendukung program kerja BEM.',
      keywords: ['relasi']
    },
    'media-teknologi': {
      title: 'Bidang Media & Teknologi',
      description: 'Bidang yang bertanggung jawab dalam pengelolaan media, informasi, dan pengembangan teknologi di BEM.',
      keywords: ['media teknologi', 'media-teknologi', 'medtek']
    },
    'minbat': {
      title: 'Bidang Minat & Bakat',
      description: 'Bidang yang bertanggung jawab dalam pengembangan minat dan bakat mahasiswa, serta mengorganisir berbagai kegiatan ekstrakurikuler dan kompetisi.',
      keywords: ['minat', 'bakat', 'minat & bakat', 'minat dan bakat', 'minbat']
    }
  };
    ...
    const fetchBidangData = async () => {
      ...
      try {
        ...
        const currentBidang = bidangList.find(bidang => {
          ...
          return config.keywords.some(k => slug.includes(k) || name.includes(k));
        });

        if (!currentBidang) {
          throw new Error(`Bidang "${config.title}" tidak ditemukan di database`);
        }
        ...
        const bidangKategoris = kategoriList.filter(k => k.bidangId === currentBidang.id);
        ...
        if (bidangSlug === 'inti') {
          ...
          const dinasKategoris = bidangKategoris.filter(k => 
            !k.nama_dinas.toLowerCase().includes('koorbid')
          );
