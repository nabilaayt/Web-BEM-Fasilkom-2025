...
const sites = [
  ...
  },
  {
    ...
    categories: [
      "Seputar Fakultas Ilmu Komputer UNSRI",
      "Info akademik",
      "Info non akademik",
    ],
    img: TetapInformasi,
...
const Inside_BEM_Apps = () => {
  ...
  return (
    <section
      id="inside-bem-apps"
      className="text-sm md:text-base px-4 sm:px-8 lg:px-16 py-2 md:py-20"
    >
      <div className="max-w-[1400px] mx-auto px-4 sm:px-8 ">
        <h2 className="text-2xl md:text-4xl font-gotham-bold text-center text-gray-900 mb-4">
          Inside <span className="text-[#4a0000]">BEM</span> Apps
        </h2>
        <p className="text-center text-gray-700 mb-8 max-w-2xl mx-auto font-gotham-book leading-relaxed">
        ...
        <div className="flex items-center justify-center gap-4 md:gap-8 mb-8">
          {sites.map((s) => (
            <button
              ...
              className={`text-base md:text-lg font-medium py-2 px-1 border-b-2 ${
                active === s.id
                  ? "border-[#4A0000] font-gotham-medium text-[#4A0000]"
                  : "border-transparent font-gotham-book text-gray-600 hover:text-[#4A0000]"
              }`}
        ...
        <div className="flex flex-col md:flex-row items-start gap-12">
          ...
          <div className="md:w-1/2">
            <div className="flex items-start gap-4 mb-4">
              ...
              <div>
                <h3 className="font-gotham-bold text-xl md:text-2xl text-gray-900 mb-2">
                  {current.title}
                </h3>
                <p className="text-gray-700 mb-4 text-sm md:text-base leading-relaxed font-gotham-book">
                  {current.description}
                </p>
              </div>
            </div>
            ...
            <div className="mb-10 flex justify-center md:hidden">
              <div className="rounded-[1.5rem] p-4 md:p-6 bg-white/60 shadow-lg">
                <img
                  src={current.img}
                  alt={current.title}
                  className="w-full h-[180px] max-w-[640px] rounded-none object-cover"
                />
              </div>
            </div>

            <h4 className="text-center md:text-left font-gotham-medium mb-2">
              Kategori {current.detail.split(" ").pop()}:
            </h4>
            <ol className="text-center md:text-left list-decimal list-inside text-gray-700 mb-6 space-y-1 font-gotham-book">
            ...
            >
              <button className="bg-[#4A0000] font-gotham-medium text-white px-6 py-3 rounded-md font-medium hover:bg-transparent hover:border-[#4A0000] hover:border hover:text-[#4A0000] transition">
                More Info
              </button>
            </a>
          </div>
